...
def build_prompt(state, ind1h, ind15m, df1h, df15, mode='trend'):
    ...
    # 환경정보 및 조건 안내
    template = f"""
f"당신은 {SYMBOL} 선물 트레이딩 전문가입니다."

{header}
아래 JSON 형식의 지표 데이터를 참고하여, 조건이 모두 충족될 경우 '롱' 또는 '숏' 시그널을 제안하고,
...
- 15분봉 기준
 ...
 진입 체크리스트 (충분히 충족될 때만 진입):
- MA 5/20/60 배열 및 골든/데드크로스 여부
- RSI(14) 기준 50선 돌파 or 이탈
- MACD 크로스 + 히스토그램 전환
- 볼린저밴드 중심선 돌파 + 수축 해제
- 명확한 매물대 지지/저항
- 평균 대비 거래량 급증
...
{prompt_json}
```
  RR ≥ {TARGET_RR} 미만인 경우 혹은 승률이 70%미만이라고 판단될 경우 반드시 '관망'을 반환하세요.
